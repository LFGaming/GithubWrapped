...
def fetch_contribution_data(username, token, year):
    ...
    if 'data' in data and 'user' in data['data'] and 'contributionsCollection' in data['data']['user']:
        # Initialize total contributions for the year
        total_contributions_year = 0
        
        # Get current year
        current_year = datetime.datetime.now().year
        
        if year > current_year:  # Ensure the year is valid
            print("The specified year is in the future.")
            return

        # Iterate through each week in the contribution calendar
        for week in data['data']['user']['contributionsCollection']['contributionCalendar']['weeks']:
            for day in week['contributionDays']:
                contribution_date = datetime.datetime.strptime(day['date'], "%Y-%m-%d")
                # Check if the contribution is in the desired year
